...
## Result (swept 2026-08-20)

The sweep below ran across the whole tree. Numbers before and after:

| | Before | After |
|---|---|---|
| Overall density, app code | 12.3% (2264 / 18368 lines, 245 files) | **6.9%** (1275 / 18368) |
| `///` doc blocks on properties, cases, funcs | 1049 | 0 |
| Multi-line type docs | 144 | 0 |
| File-header prose past the Xcode header | 113 lines | one line per file, where a file has no type doc |
| Test-target comment lines | 409 | 320 |
| Files where comments exceed a third of code | 3 | 0 |
| Comment blocks of 3+ lines | 1 | 0 |

244 files changed, verified comment-only: stripping every comment from both revisions
yields identical code.

The densest survivors are the protected material — `CollapsingBackdropHeader` (24%,
geometry), `RemoteImage`, `PosterDetailView`, `DetailSearchHost` (the field's flight),
`PersistenceCoordinator+TV` (CloudKit ordering). That is the floor, not slack.
...
Also run `python3 Scripts/find_long_comments.py .` — it must report 0 blocks of 3+ lines.

Target: single-digit density overall, and no file where comments outnumber a third of its code.
Both were met on 2026-08-20.
