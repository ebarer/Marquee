...
/// A poster's average colour, used to tint a detail screen. Memoized per path and served from
...
enum PosterTint {
    ...
    /// Poster sizes the app displays, likeliest-cached first. Any of them averages to the same
    /// colour, so whichever the caller already loaded spares a download.
    ...
    static func resolve(forPath path: String?) async -> Color? {
        ...
        // Decode here rather than leaning on `averageColor(from: Data)`: its accent fallback
        ...
        let color = Color.averageColor(from: image)
        byPath[path] = color
        return color
    }
    ...
    private static func localColor(forPath path: String) -> Color? {
        for size in cachedSizes {
            ...
            if let image = RemoteImageCache.shared.image(for: url) {
                return Color.averageColor(from: image)
            }
